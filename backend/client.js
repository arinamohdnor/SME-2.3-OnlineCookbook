...
const values = `INSERT INTO users (username, password, email, first_name, last_name, user_img)
...
pgclient.query(values, (err, res) => {
    if (err) throw err
});

pgclient.query('SELECT * FROM recipes', (err, res) => {
    if (err) throw err
    console.log(err, res.rows) // Print recipes
    pgclient.end()
});